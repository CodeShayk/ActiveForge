...
using Npgsql;
using NpgsqlTypes;
using Turquoise.ORM.Adapters.PostgreSQL;
...
namespace Turquoise.ORM
{
    ...
    public class PostgreSQLConnection : DBDataConnection
    {
        ...
        /// <summary>
        /// Wraps a field-list+FROM clause in a LIMIT expression.
        /// E.g. <c>"Name, Price FROM Products WHERE …"</c> → <c>"SELECT Name, Price FROM Products WHERE … LIMIT 10"</c>.
        /// </summary>
