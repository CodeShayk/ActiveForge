    ...
    public abstract class DBDataConnection : DataConnection, IDisposable
    {
        ...
        protected int QueryCount(Record obj, QueryTerm term, Type[] expectedTypes, FieldSubset subsetIn, Dictionary<Type, FieldSubset> expectedTypeFieldSubsets)
        {
            lock (_syncRoot)
            {
                ...
                {
                    if (fieldSubset == null)
                        fieldSubset = obj.FieldSubset(ActiveForge.FieldSubset.InitialState.IncludeAll);
            ...
            IReadOnlyList<JoinOverride> joinOverrides = null)
        {
            ...
            foreach (var fb in fieldBindingSubset)
            {
                ...
                if (fb.MapNode != null && fb.MapNode.Alias.Length > 0)
                    fields.Append(fb.MapNode.Alias).Append(GetSourceNameSeparator());
        ...
        protected string JoinSQLFromJoinSpecifications(List<JoinSpecification> specs, bool withUpdateLock, Dictionary<Type, RecordParameterCollectionBase> concreteTypeObjectParams)
        {
            ...
            foreach (var join in specs)
            {
                ...
                sb.Append(' ').Append(join.TargetAlias);
                if (withUpdateLock) sb.Append(' ').Append(GetUpdateLock());
                sb.Append(" ON ");
                sb.Append(join.SourceAlias).Append(GetSourceNameSeparator()).Append(QuoteName(join.JoinSourceField));
                sb.Append(" = ");
                sb.Append(join.TargetAlias).Append(GetSourceNameSeparator()).Append(QuoteName(join.JoinTargetField));
            }
            return sb.ToString();
        }
